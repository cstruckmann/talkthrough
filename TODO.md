...
- [ ] `ChangesetCollector`: get `git diff <base>...HEAD` (use `simple-git` or child_process), configurable `talkthrough.baseRef`, fallback to working-tree-vs-HEAD when no commits ahead. Handle: not-a-repo (readable error), empty diff (info message), binary files (skip), per-file truncation over threshold with a note in the payload.
- [ ] Define TourScript JSON schema (per PROJECT.md) + validator (zod). Unit tests: valid/invalid fixtures.
...
- [ ] `TTSEngine` interface `synthesize(text, voice) => audioFile`; cache keyed by hash(text+voice+engine) in `globalStorageUri`.
- [ ] Engine: `system` — macOS `say` → WAV (document Windows/Linux gap for now; fail with actionable message pointing to API TTS).
- [ ] Engine: `openai` TTS via user key in SecretStorage.
